...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'foolhardy':
				dialogue = [
				':dad:Dam brats trespassing with their damn noise cannons...',
				":dad:You should know it's dangerous to wander in the dark...",
				":dad:But if you want a show.. I'll give you a show!"
				];
			case 'foolhardy b-side':
				dialogue = [
				":dad:Well, even after making me disappear last time, you're back for more huh?",
				":dad:Just know that i ain't going easy on you kid.", 
				":dad:Let's get this over with."
				];
		}
		...
		switch(SONG.stage)
		{
			...
			default:
			{
				if (SONG.song.toLowerCase() != 'foolhardy' && SONG.song.toLowerCase() != 'foolhardy b-side')
				{
					defaultCamZoom = 0.9;
					curStage = 'stage';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stageback'));
				...
				else
				{
					defaultCamZoom = 0.9;
					curStage = 'zardymaze';
					var idk:FlxSprite = new FlxSprite(-600,-400);
					if (!FlxG.save.data.optimization)
					{
					idk.frames = Paths.getSparrowAtlas('Maze','shared');
					idk.animation.addByPrefix('idle', 'Stage');
					idk.animation.play('idle',true);
					}
					else
					{
					idk.loadGraphic(Paths.image('mazebutoptimized','shared'));
					}
					add(idk);
				}
			}
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + (FlxG.save.data.bside ? "" : " Terribly Hard") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
	...
	*/
	...
	function doTheThing(tween:FlxTween):Void
	{
		strumLineNotes.forEach(function(note:FlxSprite){
			FlxTween.linearMotion(note, note.x,note.y, note.x + 100, note.y,1,true, {type: FlxTween.PINGPONG});
		});
	}
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'foolhardy')
		{
			switch (curStep)
			{
				...
				case 2432:
				{
					FlxTween.tween(dad, {alpha: 0.5}, 0.5);
					strumLineNotes.forEach(function(note:FlxSprite){
						FlxTween.linearMotion(note, note.x, note.y, note.x - 50, note.y, 1, true, {onComplete: doTheThing});
					});
				}
				case 2955:
		...
		if (SONG.song.toLowerCase() == 'foolhardy b-side')
		{
			switch (curStep)
			{
				case 1:
